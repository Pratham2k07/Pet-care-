...
export default function ScheduleScreen({ navigation }: any) {
  ...
  const handleSave = () => {
    ...
    if (!wakeTime.trim()) {
      setErrorMsg('Please provide a Wake Up Time.');
      return;
    }
    if (!breakfast.time.trim() && !lunch.time.trim() && !dinner.time.trim() && validWalks.length === 0) {
      setErrorMsg('Please add at least one meal or walk time.');
      return;
    }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      ...
      >
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          {/* Diet & Hydration Baseline */}
          <Text style={styles.sectionTitle}>Diet & Baseline</Text>
          <View style={[styles.card, { zIndex: 25 }]}>
            
            <View style={styles.formGroup}>
              <Text style={styles.label}>Food Type</Text>
              <View style={styles.chipRow}>
                {['Packaged Food', 'Homemade Food', 'Mixed Diet', 'Vet-Prescribed Diet'].map((type) => (
                  <TouchableOpacity 
                    key={type}
                    style={[styles.chip, foodType === type && styles.chipActive]}
                    onPress={() => setFoodType(type)}
                  >
                    <Text style={[styles.chipText, foodType === type && styles.chipTextActive]}>{type}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            <View style={styles.formGroup}>
              <Text style={styles.label}>{foodType === 'Homemade Food' ? 'Main Ingredients' : 'Brand Name / Product'}</Text>
              <TextInput 
                style={styles.input} 
                placeholder={foodType === 'Homemade Food' ? "e.g. Chicken, rice, carrots" : "e.g. Pedigree Adult, Royal Canin"}
                placeholderTextColor="#9CA3AF"
                value={foodDetails}
            ...
            <View style={styles.formGroup}>
              <Text style={styles.label}>Average Quantity per Day</Text>
              <TextInput 
                style={styles.input} 
                placeholder="e.g. 2 bowls/day" 
                placeholderTextColor="#9CA3AF"
                value={avgQuantity}
            ...
            <View style={styles.formGroup}>
              <Text style={styles.label}>Estimated Water Intake</Text>
              <TextInput 
                style={styles.input} 
                placeholder="e.g. 1 liter/day" 
                placeholderTextColor="#9CA3AF"
                value={waterIntake}
          ...
          {errorMsg ? (
            <Text style={styles.errorTextMain}>{errorMsg}</Text>
          ) : null}
